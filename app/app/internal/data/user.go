...
// UpdateUserBalance .
func (ui *UserInfoRepo) UpdateUserBalance(ctx context.Context, userId int64, amount uint64, coinType string) error {
	if "USDT" == coinType {
		res := ui.data.DB(ctx).Table("user_balance").Where("user_id=?", userId).
			Updates(map[string]interface{}{"balance_usdt": amount})
		if res.Error != nil {
